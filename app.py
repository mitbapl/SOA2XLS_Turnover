...
def process_hdfc(f):
    ...
    try:
        ...
        exclusion_keywords = ['page:', 'account status', 'total', 'inward clg', 'opening balance', 
                      'statement of a/c', 'statement summary', 'generated on', 'generated by', 
                      'computer generated statement', 'not']
        df = df[~df.apply(lambda row: row.astype(str).str.lower().str.contains('|'.join(exclusion_keywords)).any(), axis=1)]
